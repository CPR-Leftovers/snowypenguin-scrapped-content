package fl.controls
{
   ...
   public class List extends SelectableList implements IFocusManagerComponent
   {
      ...
      {
         drawNow();
         var _loc2_:uint = Math.floor((_verticalScrollPosition + availableHeight) / this.rowHeight) - 1;
         var _loc3_:uint = Math.ceil(_verticalScrollPosition / this.rowHeight);
         if(param1 < _loc3_)
         {
      ...
      {
         var _loc1_:* = contentHeight != this.rowHeight * length;
      ...
      override protected function drawList() : void
      {
         ...
         _loc1_.y = Math.floor(_verticalScrollPosition) % this.rowHeight;
         listHolder.scrollRect = _loc1_;
         listHolder.cacheAsBitmap = useBitmapScrolling;
         var _loc2_:uint = Math.floor(_verticalScrollPosition / this.rowHeight);
         var _loc3_:uint = Math.min(length,_loc2_ + this.rowCount + 1);
         ...
         while(_loc4_ < _loc3_)
         {
            ...
            _loc6_.y = this.rowHeight * (_loc4_ - _loc2_);
            _loc6_.setSize(availableWidth + _maxHorizontalScrollPosition,this.rowHeight);
            _loc10_ = this.itemToLabel(_loc5_);
            _loc11_ = null;
            if(this._iconFunction != null)
            {
               _loc11_ = this._iconFunction(_loc5_);
            }
            else if(this._iconField != null)
            {
               _loc11_ = _loc5_[this._iconField];
            }
            if(!_loc9_)
            {
               _loc6_.data = _loc5_;
      ...
      {
         var _loc4_:int = Math.max(Math.floor(this.calculateAvailableHeight() / this.rowHeight),1);
         var _loc5_:int = -1;
         var _loc6_:int = 0;
         switch(param1)
         {
